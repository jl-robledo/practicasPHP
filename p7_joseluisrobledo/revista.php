...
if (isset($_POST['enviar'])){
    ...
    //Control de errores extra
	if(!empty($extra)){
		//limpiamos o verificamos que es una cadena
		$extra = filter_var($extra, FILTER_SANITIZE_STRING);
	} else {
		$errores .= "Por favor ingresa contenido adicional.". '\n';
		$enviado = false;
	}
	...
	if($enviado == false){
		...
		include "conexion.php";
			...
			if ($control_titulo == false){ // sacamos el mensaje si se ha encontrado el mail
				...
				echo "<script>alert('Revista registrada. Intente con otro titulo.'); </script>";
                ...
                */
                ...
                if($resultado1 == true){
                    ...
                    $query3 = "INSERT INTO magazine (Id_mag, AddResources, Id_Rm) 
                         VALUES (null, '$extra', '$id_rm' )";

					$resultado3 = $conexion->query($query3);

                	if ($resultado3 == true){
                     // Saludo de todo correcto
                        echo "<script>alert('Revista registrada:  $titulo '); </script>";
                	}
